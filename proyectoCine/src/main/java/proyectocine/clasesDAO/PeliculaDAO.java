...
public class PeliculaDAO implements DAO<Pelicula, Integer> {
  ...
  public Pelicula getLastInsertedPelicula() {
    ...
    try (Connection con = ConnectionPool.getInstance().getConnection();
      ...
      throw new RuntimeException("Error al obtener �ltima pel�cula insertada", ex);
    }
  ...
  public List<Pelicula> getPeliculasEnCartelera() {
    String query = "SELECT * FROM pelicula WHERE is_cartelera = true";
    List<Pelicula> listaPeliculasEncartelera = new ArrayList<>();
    try (Connection con = ConnectionPool.getInstance().getConnection();
        PreparedStatement preparedStatement = con.prepareStatement(query)) {
      try (ResultSet resultSet = preparedStatement.executeQuery()) {

        while (resultSet.next()) {
          listaPeliculasEncartelera.add(rsRowToPelicula(resultSet));
        }
      }
    } catch (SQLException ex) {
      throw new RuntimeException(ex);
    }
    UtilExceptions.checkListaVacia(listaPeliculasEncartelera, "ERROR CON LA LISTA GENERADA");
    return listaPeliculasEncartelera;
  }
